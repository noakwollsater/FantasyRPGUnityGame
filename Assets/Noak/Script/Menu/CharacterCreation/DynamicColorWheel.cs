...
namespace Unity.FantasyKingdom
{
    public class DynamicColorWheel : CharacterCreation, IPointerDownHandler, IDragHandler
    {
                ...
                for (int y = 0; y < size; y++)
                {
                    ...
                    else
                    {
                        texture.SetPixel(x, y, mode switch
                        {
                            ...
                            _ => AngleToRGB(angle, radius) // <-- modified call
                        });
        ...
        private Color AngleToRGB(float angle, float radius)
        {
            float hue = (angle + Mathf.PI) / (2 * Mathf.PI); // Normalize angle to [0,1]
            float saturation = Mathf.Clamp01(radius);         // Radius controls saturation
            float value = 1.0f - (radius * 0.5f);             // Add brightness falloff toward center
        ...
        private void UpdateColor(PointerEventData eventData)
        {
            ...
            {
                Debug.LogWarning("Inga delar valda att färga. Använd t.ex. ChangeSkinColor() först.");
                return;
            }
        ...
        private void ApplyColorChange(Color color, params string[] propertyKeywords)
        {
            ...
            var selectedProperties = _allColorProperties
                .Where(prop =>
                    propertyKeywords.Any(keyword =>
                        prop.Name.ToLower().Contains(keyword.ToLower())) &&
                    // Exclude facial hair if we're not targeting it
                    !(propertyKeywords.Contains("hair") && prop.Name.ToLower().Contains("facial")))
                .ToList();
